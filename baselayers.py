...
Basic MLP type layers and constructs.
'''
...
class HiddenLayer(object):
    ...
    def __init__(self, rng, input, n_in, n_out, activation, W=None, b=None,
        ...
        if W is None:            
            if (activation is not None) and (activation.func_name.lower() == 'relu'):
...
class MLPDropout(object):
    ...
    def __init__(self, rng, input, layer_sizes, dropout_rates, 
        activations, use_bias=True, classifier=False):
        ...
        OutputType = None
        if classifier:
            OutputType = LogisticRegression
        else:
            OutputType = LinearRegression

        n_in, n_out = self.weight_matrix_sizes[-1]

        dropout_output_layer = OutputType(
            input=next_dropout_layer_input, n_in=n_in, n_out=n_out)

        self.dropout_layers.append(dropout_output_layer)

        # Again, reuse paramters in the dropout output.
        output_layer = OutputType(
            input=next_layer_input,
            # scale the weight matrix W with (1-p)
    ...
    def predict(self, new_data):
        ...
        for i, layer in enumerate(self.layers):
            if i < len(self.layers) - 1:
                ...
                if self.classifier:
                    yhat = T.nnet.softmax(T.dot(next_layer_input, layer.W) + layer.b)
                    y_pred = T.argmax(yhat, axis=1)
                else:
    ...
    def predict_proba(self, new_data):
        ...
        for i,layer in enumerate(self.layers):
            if i<len(self.layers)-1:
                ...
                if self.classifier:
                    y_pred = T.nnet.softmax(T.dot(next_layer_input, layer.W) + layer.b)
                else:
                    y_pred = T.dot(next_layer_input, layer.W) + layer.b
        return y_pred
...
class LinearRegression(object):
    """Multi-out Linear Regression Class
